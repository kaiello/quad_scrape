#!/usr/bin/env python3
"""
Integrated Docling Extractor v2 (Fixed)
Combines:
1. Robust OCR & TableFormer (Standard Pipeline)
...
Fixes:
- Corrected ImageRef access (uses .pil_image instead of .image)
- Added robust safety checks for missing images
"""
...
def get_configured_converter(use_vlm: bool = False) -> DocumentConverter:
    ...
    if use_vlm:
        ...
        # CRITICAL: Must be True to extract images for annotation/figures
...
def annotate_pages(doc: DoclingDocument, output_dir: Path):
    ...
    for page_no, page in doc.pages.items():
        # FIX: Check if page.image exists AND has the .pil_image attribute populated
        if page.image and page.image.pil_image:
            try:
                # FIX: Access the actual PIL image via .pil_image
                ...
                img.save(out_path)
                _log.debug(f"Saved annotation for page {page_no}")
            except Exception as e:
                _log.warning(f"Failed to annotate page {page_no}: {e}")
        else:
            _log.warning(f"Page {page_no} has no image data. Skipping annotation.")
    ...
    for i, picture in enumerate(doc.pictures):
        try:
            if picture.prov:
                ...
                # FIX: Check for .pil_image availability
                if page_obj and page_obj.image and page_obj.image.pil_image:
                    # Crop using bbox
                    bbox = picture.prov[0].bbox.as_tuple()
                    # FIX: Access .pil_image
                    cropped_fig = page_obj.image.pil_image.crop(bbox)
                    
                    fig_path = figures_dir / f"{base_name}_figure_{i+1}.png"
                    cropped_fig.save(fig_path)
                    _log.debug(f"Exported Figure {i+1}")
                else:
                    _log.warning(f"Skipping Figure {i+1}: Page image not available.")
        except Exception as e:
    ...
    for item, level in docling_doc.iterate_items():
        ...
        if isinstance(item, TableItem):
            try:
                ...
                _log.warning("Failed to export table content for JSON.")
